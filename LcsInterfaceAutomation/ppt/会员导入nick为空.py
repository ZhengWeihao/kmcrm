#coding=utf-8
import time,datetime,requests,json,sys,ConfigParser
...
import_url='http://crm8.superboss.cc/member/memberImport'
files = {'file': open('D:\LcsInterfaceAutomation\TestFiles\ExportOrderList201712191109.csv', 'rb')}
try:
    assert json.loads(requests.post(import_url,headers=header,files=files).content)['result']==u'1'
except AssertionError:
    print u'导入失败'
i=20
while i>1:
    search_url='http://crm8.superboss.cc/member/getLastImportResult?nick=&pageNo=1&pageSize=100000'
    data=json.loads(requests.get(search_url,headers=header).content)['data']['list']
    if data==[]:
        sleep(5)
        i=i-1
    else:
